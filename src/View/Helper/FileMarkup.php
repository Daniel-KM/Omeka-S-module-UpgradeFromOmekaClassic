 ...
 * @todo Is replaced by $media->render() and various renderers.
 *
...
class FileMarkup extends AbstractHelper
{
    ...
    //         $html = link_to_file_show($linkAttributes, $html, $file);
            $html = $upgrade->link_to_file_show($linkAttributes, $html);
    ...
    //             $derivative = $linkToFile;
            if (is_string($linkToFile) && $file->hasThumbnails()) {
                $derivative = $upgrade->mapDerivative($linkToFile);
                $fileUrl = $file->thumbnailUrl($derivative);
            } else {
                $derivative = 'original';
    ...
    //         $html = '<a ' . tag_attributes($linkAttributes) . '>' . $html . '</a>';
            $html = '<a ' . $upgrade->tag_attributes($linkAttributes) . '>' . $html . '</a>';
        }
        return $html;
    }
    ...
    //     $escapedUrl = html_escape($url);
        ...
        $escapedUrl = $upgrade->html_escape($url);
        $attrs = array(
            'src' => $url,
            'class' => 'omeka-media',
            'width' => $options['width'],
    ...
    //         . '<a href="' . $escapedUrl . '">' . metadata($file, 'display_title') . '</a>'
        $html = '<' . $type . ' ' . $upgrade->tag_attributes($attrs) . '>'
            . '<a href="' . $escapedUrl . '">' . $file->displayTitle() . '</a>'
            . '</'. $type .'>';
        return $html;
    }
    ...
    //     $html = '<img ' . tag_attributes($imgAttributes) . ' />';
        $html = '<img ' . $upgrade->tag_attributes($imgAttributes) . ' />';
    ...
    //                . '>' . html_escape($file->original_filename) . '</div>';
            $html .= '<div ' . $upgrade->tag_attributes((array)$options['filenameAttributes'])
                   . '>' . $upgrade->html_escape($file->source()) . '</div>';
        }
    ...
    public function derivativeImage($file, array $options=array())
    {
        ...
        $imgClasses = array(
            ...
            'fullsize'=>'full',
            // Added for compatibility in Omeka S.
            'square' => 'thumb',
            'medium' => 'thumb',
            'large' => 'full',
        );
    ...
    //     $html .= !empty($imgHtml) ? $imgHtml : html_escape($file->original_filename);
        $html .= !empty($imgHtml) ? $imgHtml : $upgrade->html_escape($file->source());
    ...
    //         $mimeTypeClassName = str_replace(array('/', '+', '.'), '-', $file->mime_type);
            $mimeTypeClassName = str_replace(array('/', '+', '.'), '-', $file->mediaType());
    ...
    //     $wrapper = !empty($wrapperAttributes) ? '<div ' . tag_attributes($wrapperAttributes) . '>' : '';
        $wrapper = !empty($wrapperAttributes) ? '<div ' . $upgrade->tag_attributes($wrapperAttributes) . '>' : '';
        $html = !empty($wrapper) ? $wrapper . $html . "</div>" : $html;

    //     return apply_filters(
        return $upgrade->apply_filters(
            'file_markup',
            $html,
            array(
                'file' => $file,
    ...
    //     if (!($record && $record instanceof Omeka_Record_AbstractRecord)) {
        if (!($record && $record instanceof AbstractResourceEntityRepresentation)) {
            return false;
        }
    ...
    //         return false;
    //     }
        // In Omeka S, the media representation has no primary media currently.
        if (get_class($record) == 'Omeka\Api\Representation\MediaRepresentation') {
            $file = $record;
        } elseif (method_exists($record, 'primaryMedia')) {
            $file = $record->primaryMedia();
            if (empty($file)) {
                return false;
            }
        } else {
    ...
    //         $format = (get_option('use_square_thumbnail') == 1) ? 'square_thumbnail' : 'thumbnail';
            $format = ($upgrade->get_option('use_square_thumbnail') == 1) ? 'square' : 'medium';
        }

        $format = $upgrade->mapDerivative($format);
    ...
    //        $uri = $file->getWebPath($format);
        if ($file->hasThumbnails()) {
            ...
            $uri = $upgrade->img($this->_getFallbackImage($file));
        }
        $props['src'] = $uri;
    ...
    //     } else if ($fileTitle = metadata($file, 'display title', array('no_escape' => true))) {
        } elseif ($fileTitle = $file->displayTitle()) {
            $alt = $fileTitle;
        }
        $props['alt'] = $alt;
    ...
    //     return '<img ' . tag_attributes($props) . '>';
        return '<img ' . $upgrade->tag_attributes($props) . '>';
    }
